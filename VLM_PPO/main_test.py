...
def main():
    ...
    print("--------------this is the test for success rate -----------------")
    args = get_args()
    ...
    #-----------------
    model_path = "/home/mmd/RL4VLM/Evaluate_sft/llava-v1-merged-ezpoint_checkpoint"
    model_base = "liuhaotian/llava-v1.5-7b"
    model_name = get_model_name_from_path(model_path)
    tokenizer, base, image_processor, context_len = load_pretrained_model(
    ...
    # print("----------------promtssdfsfsdf-------------------------------------------")
    # print(f"00000000000qs: {qs}\n")
    # print(f"000000000500: {conv}\n")
    # print(f"00000000000: {conv.roles[0]}\n")
    # print(f"11111111111: {conv.roles[0]}\n")
    # print(prompt)
    # print("----------------promtssdfsfsdf-------------------------------------------")
    ...
    for j in tqdm(range(num_updates)):
        ...
        for step in tqdm(range(args.num_steps)):
            ...
            ################################################################
            # image = obs.cpu().squeeze(0).permute(2, 0, 1).numpy()
            # image = np.transpose(image, (1, 2, 0))
            # image_pil = Image.fromarray((image * 255).astype(np.uint8))
            # image_pil.save('obs.png')
            # plt.imshow(obs.cpu().squeeze(0))
            # plt.axis('off')  # Remove axis for better visual
            # plt.savefig('image_obs.png', bbox_inches='tight', pad_inches=0)
            # plt.show()
            # exit()
            ################################################################
        ...
        print(f"------------------------success rate =  {np.mean(episode_success_rate)}-------------------------------")
        ...
        if len(episode_rewards) > 1:
            ...
            print("reward madx :" , rollouts.rewards.max().item())
            if args.use_wandb:
